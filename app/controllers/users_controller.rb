class UsersController < ApplicationController
  ...
  def new
    prepare_registration_form(partner: params[:partner])

    return if registration_partner_valid?

    redirect_to guide_path, alert: "初期相棒を選択してください。"
  end
  ...
  def create
    ...
    unless registration_partner_valid?
      return redirect_to guide_path, alert: "初期相棒を選択してください。"
    end
  ...
  def prepare_registration_form(partner: nil)
    form_params = params.fetch(:registration, {}).permit(:job, :partner, :nickname)
    @registration_job = form_params[:job].to_s
    @registration_choice = (partner.nil? ? form_params[:partner] : partner).to_s
    @nickname = form_params[:nickname].to_s
    @registration_errors ||= []
  end

  def registration_partner_valid?
    RegistrationChoiceCatalog.valid_choice?(@registration_choice)
  end
